...
- 同步血战模式中，同一轮猜对的玩家并列，按该轮开始时的名次获得基础分。例如两人并列第 1 名，下一轮首位猜对者为第 3 名。
- 非同步血战模式仍按猜对的先后顺序排名。
- 同步普通模式的多名胜者分别计算首猜、本命及快速猜中奖励；队友共享猜测次数，但只有实际猜对的玩家获得个人胜利分。

兼容客户端可在 `playerGuess` 中附带 `sharedMetaTags` 数组。服务器仅在猜测被接受后、同步轮推进前登记标签，并通过 `tagBanApplied: true` 确认；原有独立标签事件继续支持旧客户端。
